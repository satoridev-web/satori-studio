...
## Codex Guardrail: Underscore Template Safety (Required)

When modifying any Underscore.js templates (e.g. `includes/ui-js-templates.php`), Codex MUST follow the guardrail document:

- `SATORI-STUDIO-CODEX-GUARDRAILS-UNDERSCORE-TEMPLATES-v1.0.md`

Minimum requirements:
- Never leave orphaned `<# } #>` / `<% } %>` blocks.
- Prefer emptying conditional bodies with a no-op comment over deleting conditional structure.
- Run a Builder smoke test (no console errors) before PR submission when builder templates change.
